...
import {
  ...
  PointMetadata,
  RenderMode,
} from "scatter-gl"; // adjust these imports based on your project structure
...
const Projector = () => {
  ...
  useEffect(() => {
    const getAsync = async () => {
      const data = (await getProjectorData(
        datasetId,
        apiToken
      )) as ProjectorData;
      // resetPoints();
    ...
    const setupAsync = async () => {
      ...
      const params: ScatterGLParams = {
        onClick: (pointIndex: number | null) => {
          ...
          setCanvasState(
            pointIndex === null ? CanvasState.IDLE : CanvasState.CLICK
          );
        },
    ...
    <Box id="trends-projector" height="100%" width="100%">
      <VStack style={{ flex: 1, width: "100%", height: "100%" }}>
        ...
        <Stack direction={{ base: "column", md: "row" }}>
          ...
          <Button rounded={"full"} onClick={selectRandomHandler}>
            Select Random
          </Button>
          <div ref={messagesRef} />
        </Stack>

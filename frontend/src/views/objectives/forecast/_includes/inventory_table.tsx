...
import { HStack, Box as ChakraBox, VStack, Text, Stack, Spinner, Button } from '@chakra-ui/react';
import ReactEcharts from "echarts-for-react";
import { useAuth0AccessToken } from '../../../../utilities/hooks/auth0';
import { useSelector } from 'react-redux';
import { AppState } from '../../../..';
import { getCombinedEntitiesAndInventoryTable, getEntitiesAndInventoryTables } from '../../../../utilities/backend_calls/tables';
...
export default function DataGridDemo(props: {
    ...
    const [forecastAndInventoryEntities, setForecastAndInventoryEntities] = useState<Record<string, any> | null>(null)
    const [selectedForecastAndInventoryEntities, setSelectedForecastAndInventoryEntities] = useState<Record<string, string[] | string | null>>({ "forecast_entities": null, "inventory_entity": null })
    const [forecastAndInventoyEntitiesTableData, setForecastAndInventoyEntitiesTableData] = useState<Record<string, any>[] | null>(null)
    const [forecastAndInventoyEntitiesChartData, setForecastAndInventoyEntitiesChartData] = useState<Record<string, any> | null>(null)
    const [toggleGenerateInventoryData, setToggleGenerateInventoryData] = useState(false)
    const [isGeneratingInventoryData, setIsGeneratingInventoryData] = useState(false)
    ...
    useEffect(() => {
        const getEntitiesAndInventoryTablesApi = async () => {
            const entities = await getEntitiesAndInventoryTables(props.objective_id, access_token_indexhub_api)
    ...
    useEffect(() => {
        const getInventoryDataApi = async () => {
            ...
            const forecast_inventory_table_data = await getCombinedEntitiesAndInventoryTable(props.objective_id, selectedForecastAndInventoryEntities, access_token_indexhub_api)
            setForecastAndInventoyEntitiesTableData(forecast_inventory_table_data)

            const forecast_inventory_chart_data = await getCombinedEntitiesAndInventoryChart(props.objective_id, selectedForecastAndInventoryEntities, access_token_indexhub_api)
        ...
        <>
            {forecastAndInventoryEntities ? (
                <VStack width="100%" justify="space-between" mb="1rem" mt="1rem" alignItems="flex-start">
                    <VStack width="100%">
                        <Text width="100%" fontSize="sm">Choose at least one entity and your preferred inventory to view the entity-inventory table below.</Text>
                        <HStack width="100%">
                            <ChakraBox width="50%">
                                ...
                                <Box sx={{ height: 400, width: '100%' }}>
                                    <ThemeProvider theme={MuiTheme}>
                                        <DataGrid
                                            ...
                                            pageSizeOptions={[5]}
                                            onRowSelectionModelChange={(selectedEntityIds) => {
                                                selectedForecastAndInventoryEntities["inventory_entity"] = forecastAndInventoryEntities["inventory_entities"][selectedEntityIds[0]]["entity"]
                                                setSelectedForecastAndInventoryEntities(structuredClone(selectedForecastAndInventoryEntities))
                                            }}
                                        />
                                    </ThemeProvider>
                        ...
                        <Button
                            isDisabled={!(selectedForecastAndInventoryEntities["forecast_entities"] && selectedForecastAndInventoryEntities["forecast_entities"].length > 0 && selectedForecastAndInventoryEntities["inventory_entity"])}
                    ...
                    <VStack width="100%">
                        {forecastAndInventoyEntitiesTableData && forecastAndInventoyEntitiesChartData ? (
                            ...
                            <Stack alignItems="center" borderRadius="10" justifyContent="center" height="full" backgroundColor="white" width="100%">
                                <Text>Choose your preferred entities from the tables above and click on &quot;Generate Inventory Data&quot;</Text>
                            </Stack>
                        )}
                    </VStack>
                </VStack>

...
export default function ProtectedRoute({
  ...
  useEffect(() => {
    if (isAuthenticated && access_token_indexhub_api) {
      ...
      getUserDetails(user?.sub!, access_token_indexhub_api).then(
        async (response) => {
          const status = await response.status;
          if (status == 400) {
            createUser(user, access_token_indexhub_api).then(async () => {
              // eslint-disable-next-line
              getUserDetails(user?.sub!, access_token_indexhub_api).then(
                async (response) => {
            ...
            window.alert(
              "There was an issue in retrieving your user info. Please try logging in again later"
            );
          }
        }
      );

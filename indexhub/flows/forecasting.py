...
from functime.cross_validation import expanding_window_split, select_splits
from functime.feature_extraction import add_calendar_effects, add_holiday_effects
from functime.forecasting import knn, lightgbm, snaive, zero_inflated_model
from functime.metrics import mae, overforecast, smape, underforecast
...
X_LAG = {
    "3mo": relativedelta(years=1),
    "1mo": relativedelta(years=1),
    "1w": relativedelta(months=1),
    "1d": relativedelta(months=1),
}
...
ENSEMBLE_TO_MODELS = {
    "zero_inflated:lgbm+zero_inflated:quantile_regressor": [
        "zero_inflated:lgbm",
        "zero_inflated:quantile_regressor",
    ],
    "zero_inflated:lgbm+knn": ["zero_inflated:lgbm", "knn"],
    "zero_inflated:lgbm+knn+zero_inflated:quantile_regressor": [
        "zero_inflated:lgbm",
        "knn",
        "zero_inflated:quantile_regressor",
    ],
    "zero_inflated:lgbm+snaive": ["zero_inflated:lgbm", "snaive"],
    "zero_inflated:lgbm+snaive+zero_inflated:quantile_regressor": [
        "zero_inflated:lgbm",
        "snaive",
        "zero_inflated:quantile_regressor",
    ],
    "zero_inflated:lgbm+knn+snaive": ["zero_inflated:lgbm", "knn", "snaive"],
    "zero_inflated:lgbm+knn+snaive+zero_inflated:quantile_regressor": [
        "zero_inflated:lgbm",
        "knn",
        "snaive",
        "zero_inflated:quantile_regressor",
...
def join_external_data(
    ftr_panel: pl.LazyFrame, X: pl.LazyFrame, freq: str
) -> pl.LazyFrame:
    ...
    full_idx = pl.DataFrame(itertools.product(entities, dates), columns=idx_cols).lazy()
    ...
    X_new = (
        ...
        # Resampling
        .groupby_dynamic("time", every=freq, by="entity").agg(
            [pl.sum(col) for col in X.columns if col not in ["entity", "time"]]
        )
    )
...
def transform_boolean_to_int(df: pl.LazyFrame) -> pl.LazyFrame:
    ...
    df_new = df.with_columns(
        [pl.when(pl.col(col) is True).then(1).otherwise(0).keep_name() for col in cols]
    )
    return df_new
...
def get_train_test_splits(
    ...
    cv = expanding_window_split(
        TEST_SIZE[freq], n_splits=n_splits, step_size=TEST_SIZE[freq]
    )
    splits = cv(ftr_panel)

    for i in range(n_splits):
        cols = {
            col: col.replace(f"_{i}", "")
        ...
        train_cols = [
            col for col in train.columns if col not in ["entity", "time:train"]
        ]
        test_cols = [col for col in test.columns if col not in ["entity", "time:test"]]
        train_new_cols = {col: col.replace(":train", "") for col in train.columns}
        test_new_cols = {col: col.replace(":test", "") for col in test.columns}
        n_split_to_ftr_train[i] = train.select(
            ["entity", "time:train", *train_cols]
        ).rename(train_new_cols)
        n_split_to_ftr_test[i] = test.select(
            ["entity", "time:test", *test_cols]
        ).rename(test_new_cols)
...
def _fit_and_predict(
    ...
    if use_zero_inflated:
        y_pred = zero_inflated_model(
            lags=max_lags, regressor=regressor, classifier=classifier, n_models=fh
        )(y=y, X=X, X_future=X_future, fh=fh, freq=freq)
    else:
        if model == snaive:
            forecaster = model(sp=SP[freq], fh=fh, **kwargs)
            y_pred = forecaster.fit(y=y).predict(fh=fh, freq=freq)
        else:
            if use_auto:
                forecaster = model(
                    min_lags=min_lags, max_lags=max_lags, n_models=fh, **kwargs
                )
            else:
                forecaster = model(lags=max_lags, n_models=fh, **kwargs)
            y_pred = forecaster.fit(y=y, X=X).predict(fh=fh, freq=freq, X=X_future)
...
def _split_y_and_X(
    ftr_panel: pl.LazyFrame, feature_cols: List[str] = None
...
def _get_X_future(X: pl.LazyFrame, freq: str, fh: int) -> pl.LazyFrame:
    ...
    full_idx = pl.DataFrame(itertools.product(entities, timestamps), columns=idx_cols)
    # Defensive cast dtypes to be consistent with df
    full_idx = full_idx.select(
        [pl.col(col).cast(dtypes[i]) for i, col in enumerate(full_idx.columns)]
    )
    ...
    drop_cols = [
        ...
        if col
        in ["is_weekend"]
        + [value for values in DATE_FEATURES.values() for value in values]
        or col.startswith("holidays:")
    ]
    ...
    X_future = (
        pl.concat(
            [
                full_idx.sort(idx_cols),
                X.sort(idx_cols)
                .select(pl.all().exclude(idx_cols + drop_cols))
                .collect(),
            ],
            how="horizontal",
        )
        ...
        .filter(
            pl.col("time").is_between(
                future_start_date, future_end_date, include_bounds=True
            )
        )
    ...
    country_codes = [
        col.replace("holidays:", "") for col in X.columns if col.startswith("holidays:")
    ]
    if len(country_codes) > 0:
        X_future = X_future.pipe(add_holiday_effects(country_codes, as_bool=True))
    ...
    if quantile is not None:
        ...
        if use_zero_inflated:
            ...
            if isinstance(regressor, LGBMRegressor):
                fit_kwargs["regressor"] = LGBMRegressor(
                    objective="quantile", alpha=quantile
                )
            elif isinstance(regressor, QuantileRegressor):
                fit_kwargs["regressor"] = QuantileRegressor(
                    solver="highs", alpha=quantile
                )

            fit_kwargs["classifier"] = LGBMClassifier(
                objective="quantile", alpha=quantile
            )
...
def _run_ensemble(
    ...
    if ignore_zeros:
        ...
        y_pred_mean = y_pred_mean.with_columns(
            [
                pl.when(pl.col(col) == 0).then(None).otherwise(pl.col(col))
...
def postproc(
    ...
    idx_cols = [
        col for col in y_pred.columns if col in ["model", "entity", "time", "quantile"]
    ]

    y_pred = (
        # Ensemble if there are multiple forecast columns
        _run_ensemble(y_pred, idx_cols, ignore_zeros).rename(
...
def compute_metrics(
    ...
    merged_mae = (
        manual_mae.join(forecast_mae, on="entity", how="outer", suffix=":forecast")
        .rename({"mae": "mae:manual"})
        .with_column(
            (pl.col("mae:manual") - pl.col("mae:forecast")).alias("mae_improvement")
        )
        .with_columns(
            [
    ...
    merged_underforecast = (
        ...
        .with_column(
            (
                pl.col("underforecast_improvement")
                / pl.col("underforecast:manual")
                * 100
            ).alias("underforecast_improvement_%")
        )
    )
    merged_overforecast = (
        manual_overforecast.join(
        ...
        .with_column(
            (
                pl.col("overforecast_improvement") / pl.col("overforecast:manual") * 100
            ).alias("overforecast_improvement_%")
        )
    )
    merged_smape = (
        manual_smape.join(forecast_smape, on="entity", how="outer", suffix=":forecast")
        .rename({"smape": "smape:manual"})
        .with_column(
    ...
    metrics = (
        merged_mae.join(merged_underforecast, on="entity", how="outer")
    ...
    if add_dummy_entity and dummy_entity_cols:
        for dummy_entity_col in dummy_entity_cols:
            feature_cols = feature_cols + [
                col
                for col in ftr_panel.columns
                if col.startswith(f"is_{dummy_entity_col}")
            ]

    if add_holiday:
        feature_cols = feature_cols + [
            col for col in ftr_panel.columns if col.startswith("holidays:")
        ]
...
def run_forecast_flow(inputs: RunForecastFlowInput) -> RunForecastFlowOutput:
    with pl.StringCache():
        ...
        ftr_panel_manual = (
            ...
            # Defensive cast
            .with_column(
                pl.col("target:manual").cast(
                    ftr_panel.select("target:actual").dtypes[0]
                )
            )
        )
        ...
        if inputs.external_data_paths:
            for external_data_path in inputs.external_data_paths:
                external_data = load_external_data(
                    s3_bucket=inputs.s3_tscatalog_bucket, s3_path=external_data_path
                )
                ftr_panel = join_external_data(
                    ftr_panel=ftr_panel, X=external_data, freq=freq
                )
                external_cols = external_cols + [
                    col
                    for col in external_data.columns
                    if col not in ["entity", "time", "target:actual"]
                ]
        ...
        for model, settings in MODEL_TO_SETTINGS.items():
            ...
            fit_kwargs_by_model = {**fit_kwargs, **settings}
            backtest_by_model = run_backtest(
        ...
        transf_backtests_by_model = postproc(
            y_preds=backtests_by_model, ignore_zeros=False, **uplift_postproc_kwargs
        )
        ...
        for ensemble, models in ENSEMBLE_TO_MODELS.items():
            ...
            for model in models:
                backtest = (
                    transf_backtests_by_model.filter(pl.col("model") == model)
                ...
                if ensemble_backtest is None:
                    ...
                    ensemble_backtest = ensemble_backtest.join(
                        backtest, on=["entity", "time"]
                    )
            transf_ensemble_backtest = postproc(
                y_preds=[ensemble_backtest],
                ignore_zeros=False,
                **uplift_postproc_kwargs,
        ...
        for ensemble, models in ENSEMBLE_TO_MODELS.items():
            ...
            if manual_model in models:
                continue
            backtest = backtests_by_ensemble.filter(pl.col("model") == ensemble).drop(
                "model"
            )
            metric = (
                compute_metrics(
        ...
        best_ensemble_model = (
            metrics_by_ensemble.groupby("model")
        ...
        transf_backtests_by_model = pl.concat(
            [
                transf_backtests_by_model,
                backtests_by_ensemble.filter(
                    pl.col("model") == best_ensemble_model
                ).select(
                    [
                        pl.col("entity"),
                        pl.col("time"),
        ...
        for feature, feature_settings in FEATURES_TO_COLS.items():
            ...
            if len(feature_cols) > 0:
                ...
                ftr_panel_by_feature = ftr_panel.select(
                    ["entity", "time", "target:actual", *append_feature_cols]
                )
            else:
            ...
            (
                n_split_to_ftr_train_by_feature,
                n_split_to_ftr_test_by_feature,
        ...
        for feature, backtest in ensemble_backtests_by_feature.items():
            transf_ensemble_backtest = (
                postproc(
                    y_preds=[backtest], ignore_zeros=False, **uplift_postproc_kwargs
                )
            ...
            if feature == list(ensemble_backtests_by_feature.keys())[-1]:
                manual_backtest = (
                    ftr_panel_manual.with_column(pl.lit("manual").alias("model"))
                    .rename({"target:manual": "target:forecast"})
                    .select(["entity", "time", "model", "target:forecast"])
                    .filter(
                        pl.col("time").is_in(
                            backtest.collect().get_column("time").unique()
                        )
                    )
                )
                transf_ensemble_backtest = (
                    postproc(
                        y_preds=[
                            backtest.join(
                                manual_backtest.select(pl.all().exclude("model")),
                                on=["entity", "time"],
                            )
                        ],
                        ignore_zeros=False,
                    ...
                    .with_column(pl.lit("ensemble+manual").alias("model"))
                    .select(["entity", "time", "model", "target:forecast"])
                )
                backtests_by_feature.append(transf_ensemble_backtest)
        ...
        if inputs.use_manual_zeros:
            ensemble_overrides_zero_backtest = postproc(
                y_preds=[backtests_by_feature[-1], manual_backtest],
                ignore_zeros=False,
                **postproc_kwargs,
            ).with_column(pl.lit("ensemble+manual+overrides_zero").alias("model"))
            transf_backtests_by_model = pl.concat(
                [
                    transf_backtests_by_model,
        ...
        metrics_by_model = []
        uplift_models = transf_backtests_by_model.collect().get_column("model").unique()
        ...
        for quantile in QUANTILES:
            ...
            for model in ensemble_models:
                ...
                if ensemble_backtest is None:
                    ...
                    ensemble_backtest = ensemble_backtest.join(
                        backtest, on=["entity", "time", "quantile"]
                    )

            joined_ensemble_backtest = ensemble_backtest.join(
                manual_backtest.drop("model").with_column(
                    pl.lit(quantile).alias("quantile")
                ),
                on=["entity", "time", "quantile"],
            )
            transf_ensemble_backtest = postproc(
                y_preds=[joined_ensemble_backtest],
                ignore_zeros=False,
                **uplift_postproc_kwargs,
            )
        ...
        if inputs.use_manual_zeros:
            backtests_overrides_zero = postproc(
                y_preds=[backtests], ignore_zeros=False, **postproc_kwargs
        ...
        for quantile in QUANTILES:
            ...
            for model in ensemble_models:
                ...
                if ensemble_y_pred is None:
                    ...
                    ensemble_y_pred = ensemble_y_pred.join(
                        y_pred, on=["entity", "time", "quantile"]
                    )

            joined_ensemble_y_pred = ensemble_y_pred.join(
                ftr_panel_manual.filter(
                    pl.col("time").is_in(
                        ensemble_y_pred.collect().get_column("time").unique()
                    )
                ).with_column(pl.lit(quantile).alias("quantile")),
                on=["entity", "time", "quantile"],
            )
            transf_ensemble_y_pred = postproc(
                y_preds=[joined_ensemble_y_pred],
                ignore_zeros=True,
                **uplift_postproc_kwargs,
            )
        ...
        if inputs.use_manual_zeros:
            y_preds_overrides_zero = postproc(
                y_preds=[y_preds], ignore_zeros=True, **postproc_kwargs
            ).rename({"target:forecast": "target:forecast_overrides_zero"})
